...
>(({ className, ...props }, ref) => (
    <div className="h-[40px] flex items-center gap-2 p-2 border-b border-b-neutral/25" cmdk-input-wrapper="">
        ...
        <CommandPrimitive.Input
            ref={ref}
            className={cn(
                "flex w-full rounded-sm bg-transparent text-base outline-none placeholder:text-neutral/25 disabled:cursor-not-allowed disabled:opacity-50",
                className
            )}
            {...props}
        />
...
>(({ className, ...props }, ref) => (
    <CommandPrimitive.Item
        ref={ref}
        className={cn(
            "relative flex cursor-default select-none items-center rounded-sm-sm px-2 py-1.5 text-sm outline-none",
            className
        )}
        {...props}
    />

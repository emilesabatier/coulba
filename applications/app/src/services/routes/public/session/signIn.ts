...
import { PublicSession } from "@monassosportive/schemas/routes"
import { z } from "zod"
import { ReturnResponse } from "../../returnType"


export type SignIn = {
    body: PublicSession.SignInBody
}

export async function signIn(props: SignIn): Promise<ReturnResponse<undefined>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/public/session/sign-in`),
            {
                method: "PATCH",
                headers: {

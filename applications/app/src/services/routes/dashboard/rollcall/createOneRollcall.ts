...
type CreateOneRollcall = {
    body: DashboardRollcall.CreateOneRollcallBody
}

export async function createOneRollcall(props: CreateOneRollcall): Promise<ReturnResponse<DashboardRollcall.CreateOneRollcallReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/rollcall`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.rollcall.createOneRollcallReturn.safeParse(await response.json())

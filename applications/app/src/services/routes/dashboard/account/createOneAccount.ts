...
type CreateOneAccount = {
    body: DashboardAccount.CreateOneAccountBody
}

export async function createOneAccount(props: CreateOneAccount): Promise<ReturnResponse<DashboardAccount.CreateOneAccountReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/account`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.account.createOneAccountReturn.safeParse(await response.json())

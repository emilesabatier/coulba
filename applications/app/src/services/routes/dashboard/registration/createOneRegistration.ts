...
type CreateOneRegistration = {
    params: DashboardRegistration.CreateOneRegistrationParams
    body: DashboardRegistration.CreateOneRegistrationBody
}

export async function createOneRegistration(props: CreateOneRegistration): Promise<ReturnResponse<DashboardRegistration.CreateOneRegistrationReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/registration-form/${props.params.idRegistrationForm}/registration`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.registration.createOneRegistrationReturn.safeParse(await response.json())

...
type UpdateOneCard = {
    params: DashboardCard.UpdateOneCardParams,
    body: DashboardCard.UpdateOneCardBody
}

export async function updateOneCard(props: UpdateOneCard): Promise<ReturnResponse<DashboardCard.UpdateOneCardReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/card/${props.params.id}`),
            {
                method: "PUT",
                headers: {
        ...
        const parsedResponse = dashboardSchema.card.updateOneCardReturn.safeParse(await response.json())

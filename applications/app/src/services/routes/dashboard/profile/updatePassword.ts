...
type UpdatePassword = {
    body: DashboardProfile.UpdatePasswordBody
}

export async function updatePassword(props: UpdatePassword): Promise<ReturnResponse<DashboardProfile.UpdatePasswordReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/profile/update-password`),
            {
                method: "PATCH",
                headers: {
        ...
        const parsedResponse = dashboardSchema.profile.updatePasswordReturn.safeParse(await response.json())

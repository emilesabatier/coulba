...
type CreateOneRole = {
    body: DashboardRole.CreateOneRoleBody
}

export async function createOneRole(props: CreateOneRole): Promise<ReturnResponse<DashboardRole.CreateOneRoleReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/role`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.role.createOneRoleReturn.safeParse(await response.json())

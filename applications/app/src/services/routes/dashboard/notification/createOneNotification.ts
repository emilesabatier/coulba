...
type CreateOneNotification = {
    body: DashboardNotification.CreateOneNotificationBody
}

export async function createOneNotification(props: CreateOneNotification): Promise<ReturnResponse<DashboardNotification.CreateOneNotificationReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/notification`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.notification.createOneNotificationReturn.safeParse(await response.json())

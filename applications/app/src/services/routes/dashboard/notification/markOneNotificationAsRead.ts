...
type MarkOneNotificationAsRead = {
    params: DashboardNotification.MarkOneNotificationAsReadParams,
    body: DashboardNotification.MarkOneNotificationAsReadBody
}

export async function markOneNotificationAsRead(props: MarkOneNotificationAsRead): Promise<ReturnResponse<DashboardNotification.MarkOneNotificationAsReadReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/notification/mark-one-as-read/${props.params.id}`),
            {
                method: "PATCH",
                headers: {
        ...
        const parsedResponse = dashboardSchema.notification.markOneNotificationAsReadReturn.safeParse(await response.json())

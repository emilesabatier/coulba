...
type CreateOneLesson = {
    body: DashboardLesson.CreateOneLessonBody
}

export async function createOneLesson(props: CreateOneLesson): Promise<ReturnResponse<DashboardLesson.CreateOneLessonReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/lesson`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.lesson.createOneLessonReturn.safeParse(await response.json())

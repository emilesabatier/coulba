...
type CreateOneOccurrence = {
    body: DashboardOccurrence.CreateOneOccurrenceBody
}

export async function createOneOccurrence(props: CreateOneOccurrence): Promise<ReturnResponse<DashboardOccurrence.CreateOneOccurrenceReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/occurrence`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.occurrence.createOneOccurrenceReturn.safeParse(await response.json())

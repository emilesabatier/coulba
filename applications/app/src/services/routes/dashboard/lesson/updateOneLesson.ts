...
type UpdateOneLesson = {
    params: DashboardLesson.UpdateOneLessonParams,
    body: DashboardLesson.UpdateOneLessonBody
}

export async function updateOneLesson(props: UpdateOneLesson): Promise<ReturnResponse<DashboardLesson.UpdateOneLessonReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/lesson/${props.params.id}`),
            {
                method: "PUT",
                headers: {
        ...
        const parsedResponse = dashboardSchema.lesson.updateOneLessonReturn.safeParse(await response.json())

...
type CreateOneCollaborator = {
    body: DashboardCollaborator.CreateOneCollaboratorBody
}

export async function createOneCollaborator(props: CreateOneCollaborator): Promise<ReturnResponse<DashboardCollaborator.CreateOneCollaboratorReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/collaborator`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.collaborator.createOneCollaboratorReturn.safeParse(await response.json())

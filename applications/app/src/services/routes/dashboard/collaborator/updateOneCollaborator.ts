...
type UpdateOneCollaborator = {
    params: DashboardCollaborator.UpdateOneCollaboratorParams,
    body: DashboardCollaborator.UpdateOneCollaboratorBody
}

export async function updateOneCollaborator(props: UpdateOneCollaborator): Promise<ReturnResponse<DashboardCollaborator.UpdateOneCollaboratorReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/collaborator/${props.params.id}`),
            {
                method: "PUT",
                headers: {
        ...
        const parsedResponse = dashboardSchema.collaborator.updateOneCollaboratorReturn.safeParse(await response.json())

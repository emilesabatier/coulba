...
type CreateOneEvent = {
    body: DashboardEvent.CreateOneEventBody
}

export async function createOneEvent(props: CreateOneEvent): Promise<ReturnResponse<DashboardEvent.CreateOneEventReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/event`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.event.createOneEventReturn.safeParse(await response.json())

...
type CreateOnePosition = {
    body: DashboardPosition.CreateOnePositionBody
}

export async function createOnePosition(props: CreateOnePosition): Promise<ReturnResponse<DashboardPosition.CreateOnePositionReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/position`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.position.createOnePositionReturn.safeParse(await response.json())

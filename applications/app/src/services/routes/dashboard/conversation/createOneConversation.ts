...
type CreateOneConversation = {
    body: DashboardConversation.CreateOneConversationBody
}

export async function createOneConversation(props: CreateOneConversation): Promise<ReturnResponse<DashboardConversation.CreateOneConversationReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/conversation`),
            {
                method: "POST",
                headers: {
        ...
        const parsedResponse = dashboardSchema.conversation.createOneConversationReturn.safeParse(await response.json())

...
type UpdateAssociation = {
    body: DashboardAssociation.UpdateAssociationBody
}

export async function updateAssociation(props: UpdateAssociation): Promise<ReturnResponse<DashboardAssociation.UpdateAssociationReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/association`),
            {
                method: "PUT",
                headers: {
        ...
        const parsedResponse = dashboardSchema.association.updateAssociationReturn.safeParse(await response.json())

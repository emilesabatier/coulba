...
type UpdateOneContact = {
    params: DashboardContact.UpdateOneContactParams,
    body: DashboardContact.UpdateOneContactBody
}

export async function updateOneContact(props: UpdateOneContact): Promise<ReturnResponse<DashboardContact.UpdateOneContactReturn>> {
    try {
        const response = await fetch(
            new URL(`${import.meta.env.VITE_PUBLIC_API_BASE}/dashboard/contact/${props.params.id}`),
            {
                method: "PUT",
                headers: {
        ...
        const parsedResponse = dashboardSchema.contact.updateOneContactReturn.safeParse(await response.json())

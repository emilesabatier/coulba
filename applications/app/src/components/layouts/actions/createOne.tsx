...
type CreateOne<T extends FieldValues> = {
...
export function CreateOne<T extends FieldValues>(props: CreateOne<T>) {
    ...
    return (
        <Sheet open={open} onOpenChange={(open) => setOpen(open)}>
            ...
            {!open ? null : (
                <SheetContent onInteractOutside={() => onCancel()} side={isMobile ? "bottom" : "right"} forceMount>
                    <SheetHeader>
                        <SheetTitle>{props.title}</SheetTitle>
                    </SheetHeader>
                    <FormRoot {...form}>
                        <form className="overflow-hidden grid grid-rows-[auto_min-content] gap-4 md:gap-8">
                            <div className="h-full flex flex-col justify-start items-stretch gap-2 md:gap-4 px-2 pt-2 md:px-4 md:pt-4 overflow-y-auto overflow-x-hidden">
                                {props.children}
                            </div>
